//TESTDKS JOB  CLASS=C,MSGCLASS=A,MSGLEVEL=(1,1),REGION=7000K
//*
//JOBPROC DD   DSN=K.S.V1R5M0.PROCLIB,DISP=SHR
//*
//TESTDKS EXEC  PROC=K2KCOBCL
//COPY.SYSUT1 DD *
       ID DIVISION.
       PROGRAM-ID. TESTDKS.
      ...
      * ksds direct access: read, write, delete, rewrite, unlock
       ...
       77  WS-BEGIN   PIC X(23) VALUE 'TESTDKS WORKING STORAGE'.
       ...
       01  PF-MSG.
         02  PF-MSG0  PIC X(79) VALUE 'KSDS direct test (use w/CEDF)'.
         02  PF-MSG1.
      ...
      *    GO TO DO-IT-AGAIN.
       ...
       SCRIPT-1.
           MOVE 0 TO TEST-NUM.
      * 1--- read record THREE (key = 333333). should succeed
           MOVE '333333' TO KEY77.
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM READ-1.
      * 2--- delete record THREE (key = 333333). should fail
           ADD 1 TO TEST-NUM. MOVE 2 TO EXPECTING.
           PERFORM DELETE-1.
      * 3--- rewrite record THREE (key = 333333). should fail
           ADD 1 TO TEST-NUM. MOVE 2 TO EXPECTING.
           PERFORM REWRITE-1.
      * 4--- unlock record THREE (key = 333333). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM UNLOCK-1.
      * 5--- read/update record THREEA (key = 333334). should succeed
           MOVE '333334' TO KEY77.
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM READ-UPDATE-1.
      * 6--- delete record THREEA (key = 333334). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           MOVE REC77 TO SAVREC77.
           PERFORM DELETE-1.
      * 7--- rewrite record THREEA (key = 333334). should fail
           ADD 1 TO TEST-NUM. MOVE 2 TO EXPECTING.
           PERFORM REWRITE-1.
      * 8--- unlock record THREEA (key = 333334). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM UNLOCK-1.
      * 9--- read record THREEA (key = 333334). should fail
           ADD 1 TO TEST-NUM. MOVE 2 TO EXPECTING.
           PERFORM READ-1.
      * 10-- read/update record THREEB (key = 333335). should succeed
           MOVE '333335' TO KEY77.
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM READ-UPDATE-1.
      * 11-- rewrite record THREEB (key = 333335). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM REWRITE-1.
      * 12-- unlock record THREEB (key = 333335). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM UNLOCK-1.
      * 13-- read/update record THREEB (key = 333335). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM READ-UPDATE-1.
      * 14-- read/update record THREEB (key = 333335). should fail
           ADD 1 TO TEST-NUM. MOVE 2 TO EXPECTING.
           PERFORM READ-UPDATE-1.
      * 15-- rewrite record THREEB (key = 333335). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM REWRITE-1.
      * 16-- unlock record THREEB (key = 333335). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           PERFORM UNLOCK-1.
      * 17-- WRITE record THREEA (key = 333334). should fail
      *                                          (int key <> ridfld)
           ADD 1 TO TEST-NUM. MOVE 2 TO EXPECTING.
           MOVE '333334' TO KEY77.
           PERFORM WRITE-1.
      * 18-- WRITE record THREEA (key = 333334). should succeed
           ADD 1 TO TEST-NUM. MOVE 1 TO EXPECTING.
           MOVE SAVREC77 TO REC77.
           PERFORM WRITE-1.
      * 19-- WRITE record THREEA (key = 333334). should fail (dup)
           ADD 1 TO TEST-NUM. MOVE 2 TO EXPECTING.
           PERFORM WRITE-1.
      * ---- that's all folks...
           GO TO DO-IT-AGAIN.
